...
description: Hiding seen posts, mark as seen on scroll, filtering subreddits, URL-specific overrides, and managing hidden content
===END METADATA===

# Hiding Content

Hydra offers several ways to hide content from your feeds. Filters apply to posts in the main feeds and subreddits, but do not apply to search results or user profiles. Excessive filtering may make load times slower because more posts have to be loaded before showing results.
...
This setting automatically marks posts as seen as you scroll past them, without needing to open each one. When combined with "Hide Seen Posts", posts are hidden as soon as you scroll past.

To enable it, go to [Filters settings](hydra://settings/general/filters) and toggle **"Mark as Seen On Scroll"**. You will need to restart the app for this change to take effect.
...
Beyond the options above, Hydra also supports keyword-based text filters and AI-powered smart filters for more granular control over what appears in your feed. These are configured in [Filters settings](hydra://settings/general/filters).

For details, see [Text Filters](hydra://settings/guide/?doc=text_filters) and [AI Filters](hydra://settings/guide/?doc=ai_filters).

---

Configure all hiding options in [Filters settings](hydra://settings/general/filters).
