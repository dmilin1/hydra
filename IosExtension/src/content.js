...
function runCheck() {
    ...
    if (match) {
        ...
        if (postID && commentID && context) {
            window.location.replace(`hydra://openURL?url=reddit.com/r/${subreddit}/comments/${postID}/_/${commentID}/?context=${context}`);

        } else if (postID && commentID) {
            window.location.replace(`hydra://openURL?url=reddit.com/r/${subreddit}/comments/${postID}/_/${commentID}`);

        } else if (postID) {
            window.location.replace(`hydra://openURL?url=reddit.com/r/${subreddit}/comments/${postID}`);

        } else {
            window.location.replace(`hydra://openURL?url=reddit.com/r/${subreddit}`);
